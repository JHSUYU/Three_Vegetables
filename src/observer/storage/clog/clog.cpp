  ...
  switch (flag) {
    ...
    case REDO_UPDATE: {
      if (!rec || !rec->data()) {
        ...
        log_record_.ins.rid_ = rec->rid();
        log_record_.ins.data_len_ = data_len;
        log_record_.ins.hdr_.logrec_len_ = _align8(CLOG_UPD_REC_NODATA_SIZE + data_len);
...
RC CLogRecord::copy_record(void *dest, int start_off, int copy_len)
{
  ...
  if (start_off + copy_len > get_logrec_len()) {
    ...
    int NODATA_SIZE = 0;
    if (flag_ == REDO_INSERT) NODATA_SIZE = CLOG_INS_REC_NODATA_SIZE;
    else NODATA_SIZE = CLOG_UPD_REC_NODATA_SIZE;
    if (start_off > NODATA_SIZE) {
      memcpy(dest, log_rec->ins.data_ + start_off - NODATA_SIZE, copy_len);
    } else if (start_off + copy_len <= NODATA_SIZE) {
      memcpy(dest, (char *)log_rec + start_off, copy_len);
    } else {
      memcpy(dest, (char *)log_rec + start_off, NODATA_SIZE - start_off);
      memcpy((char *)dest + NODATA_SIZE - start_off,
          log_rec->ins.data_,
          copy_len - (NODATA_SIZE - start_off));
    }
  }
  return RC::SUCCESS;
}
